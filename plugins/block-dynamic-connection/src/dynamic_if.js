...
Blockly.Blocks["dynamic_if"] = {
  ...
  mutationToDom: function () {
    ...
    const inputNames = this.inputList
      ...
      .map((input) => input.name.replace("IF", ""))
      .join(",");
  ...
  insertElseIf: function (index) {
    ...
    this.appendValueInput("IF" + caseNumber)
      .setCheck("Boolean")
      .appendField(Blockly.Msg["CONTROLS_IF_MSG_ELSEIF"], "elseif");
    this.appendStatementInput("DO" + caseNumber).appendField(
      Blockly.Msg["CONTROLS_IF_MSG_THEN"]
    );
    ...
    if (connection.targetConnection && input.name.includes("IF")) {
      ...
      if (!nextIfInput || nextIfInput.name == "ELSE") {
        ...
        const nextIfConnection =
          nextIfInput && nextIfInput.connection.targetConnection;
        if (
          nextIfConnection &&
